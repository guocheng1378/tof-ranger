package com.example.tofranger;

import java.util.ArrayDeque;
import java.util.Arrays;

/**
 * Rolling statistics tracker for distance measurements.
 * All stats use sliding window only — no all-time accumulation.
 */
public class DistanceStats {

    private final ArrayDeque<Float> window;
    private final int maxSize;

    // Cached sliding window stats
    private float windowMin = Float.MAX_VALUE;
    private float windowMax = Float.MIN_VALUE;
    private float windowSum = 0;

    private int allTimeCount = 0;

    // Sampling rate
    private long lastNanos = 0;
    private int sampleCount = 0;
    private int actualHz = 0;

    public DistanceStats(int windowSize) {
        this.maxSize = windowSize;
        this.window = new ArrayDeque<>(windowSize);
    }

    public DistanceStats() {
        this(100);
    }

    /** Add a valid distance sample */
    public void add(float mm) {
        if (mm < 0) return;

        // Evict oldest if full
        if (window.size() >= maxSize) {
            float removed = window.removeFirst();
            windowSum -= removed;
        }

        window.addLast(mm);
        windowSum += mm;
        allTimeCount++;

        // Recompute min/max from window (O(n) but small n)
        recomputeMinMax();
    }

    private void recomputeMinMax() {
        windowMin = Float.MAX_VALUE;
        windowMax = Float.MIN_VALUE;
        for (float v : window) {
            if (v < windowMin) windowMin = v;
            if (v > windowMax) windowMax = v;
        }
    }

    /** Call on every sensor event for Hz calculation */
    public void tickHz() {
        sampleCount++;
        long now = System.nanoTime();
        if (lastNanos == 0) {
            lastNanos = now;
            return;
        }
        if (now - lastNanos >= 1_000_000_000L) {
            actualHz = sampleCount;
            sampleCount = 0;
            lastNanos = now;
        }
    }

    public float getMin() {
        return window.size() > 0 ? windowMin : 0;
    }

    public float getMax() {
        return window.size() > 0 ? windowMax : 0;
    }

    /** Average over sliding window */
    public float getAvg() {
        return window.size() > 0 ? windowSum / window.size() : 0;
    }

    /** Standard deviation over sliding window */
    public float getStdDev() {
        int size = window.size();
        if (size < 2) return 0;
        float mean = windowSum / size;
        float sumSq = 0;
        for (float v : window) {
            float d = v - mean;
            sumSq += d * d;
        }
        return (float) Math.sqrt(sumSq / size);
    }

    /** Median over sliding window */
    public float getMedian() {
        int size = window.size();
        if (size == 0) return 0;
        float[] sorted = new float[size];
        int i = 0;
        for (float v : window) sorted[i++] = v;
        Arrays.sort(sorted);
        return sorted[size / 2];
    }

    public int getSampleCount() {
        return allTimeCount;
    }

    public int getWindowSize() {
        return window.size();
    }

    public int getActualHz() {
        return actualHz;
    }

    /** Trend: positive = moving away, negative = moving closer */
    public float getTrend() {
        int size = window.size();
        if (size < 10) return 0;
        int half = size / 2;
        float firstHalf = 0, secondHalf = 0;
        int i = 0;
        for (float v : window) {
            if (i < half) firstHalf += v;
            else secondHalf += v;
            i++;
        }
        return (secondHalf / (size - half)) - (firstHalf / half);
    }

    public void reset() {
        window.clear();
        windowMin = Float.MAX_VALUE;
        windowMax = Float.MIN_VALUE;
        windowSum = 0;
        allTimeCount = 0;
        sampleCount = 0;
        actualHz = 0;
        lastNanos = 0;
    }
}
